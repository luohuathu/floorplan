...
void move_3(struct fp_st_lh *root, float epsilon)//move type3: insert space between adjacent blocks
{
	if(!root || root->pos == Leaf) return;
	move_3(root->LChild, epsilon);
	move_3(root->RChild, epsilon);
...
void floor_plan_run() 
{
	...
	while(AcceptRate > RateThreshold && (float)(current_time - begin_time) < run_time)   //simulated annealing
	{
		...
		switch(coin_bit)
		{
			...
			case 3: move_3(tree_b, epsilon); break;
		}
